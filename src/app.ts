...
function displayCargos(cargos: Cargo[]): void {
...
function displayPagination(totalCargos: number): void {
...
cargoFormContainer.addEventListener('click', function(event) {
    if ((event.target as HTMLElement).id === 'btn-add-cargo') {
        ...
        if (poidsMax.value === '' || isNaN(parseFloat(poidsMax.value)) || parseFloat(poidsMax.value) <= 0) {
        ...
        if (prix.value === '' || isNaN(parseFloat(prix.value)) || parseFloat(prix.value) <= 0) {
        ...
        if (distance.value === '' || isNaN(parseFloat(distance.value)) || parseFloat(distance.value) <= 0) {
        ...
        if (valid) {
            ...
            const cargo: Cargo = {
                numero: numero,
                poids_max: parseFloat(poidsMax.value),
